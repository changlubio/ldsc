...
def read_header(fh):
    ...
    (openhandle, compression) = get_compression(fh)
    if fh.endswith('gz'):
        return [x.decode('utf-8').rstrip('\n') for x in openhandle.readline().split()]
    else:
        return [x.rstrip('\n') for x in openhandle.readline().split()]
...
def get_compression(fh):
    ...
    if fh.endswith('gz'):
        compression = 'gzip'
        openhandle = gzip.open(fh, 'rb')
    elif fh.endswith('bz2'):
        compression = 'bz2'
        openhandle = bz2.BZ2File(fh, 'r')
    else:
        openhandle = open(fh, 'r')
        compression = None

    return openhandle, compression
...
def allele_merge(dat, alleles, log):
    ...
    # dat.loc[~jj, [i for i in dat.columns if i != 'SNP']] = float('nan') # This produced unexpected error, not sure why
    dat = dat[jj]
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        if args.merge_alleles:
            ...
            (openhandle, compression) = get_compression(args.merge_alleles)
        ...
        (openhandle, compression) = get_compression(args.sumstats)
